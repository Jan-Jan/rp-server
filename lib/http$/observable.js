...
const staticRouting = require('./static').static
...
const _handleRoute = (routeDefinitions) => ({ res, ...rest }) => {
  return Object.assign({}, res, rest, { op: route(routeDefinitions)(rest) })
}


const _handleStatic = (root, options) =>
  async function(data) {
    const op = await staticRouting(data, root, options)
